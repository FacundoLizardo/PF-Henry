export const listCourses = [
    {
        course_id: 'c4796a20-8f33-42a5-aa9a-2ac3305d9172',
        title: "Curso de microlearning",
        description: "Aquí aprenderás qué es el microlearning",
        instructor_id: 'b05d6c8d-0eaa-4e99-8b75-548f5466714e',
        created_at: "20-12-2023 15:45hs",
        updated_at: "20-12-2023 15:45hs",
        progress: 0,
        category: "Programación",
        image:
            "https://i.ytimg.com/vi/MHip3eVhK9g/hq720.jpg?sqp=-oaymwEcCOgCEMoBSFXyq4qpAw4IARUAAIhCGAFwAcABBg==&rs=AOn4CLBa2Tc3wl_CWfJzbvhBmHo1sBXdEg",
        instructorImage:
            "https://yt3.ggpht.com/peu87JRecAO0qd7KhfPHLrQ_XJBEiuAiNHGuGU74dvJRnfNzP6x3sNfiIINRISZpDIqQzRgFpg=s68-c-k-c0x00ffffff-no-rj",
        rating: 4,
        comments: [
            {
                user: '0ac15268-6b06-4c1d-8c7e-0164d607ad05',
                description: "Lorem lorem lorem lorem lorem"
            },
            {
                user: 'd34e4371-e540-4b3d-9b11-45f46a6485d3',
                description: "Lorem lorem lorem lorem lorem"
            }
        ],
        lessons: [
            {
                lesson_id: 'e3c2c1a6-82ab-45ac-b12b-d08b69eb8e5d',
                title: "Introducción al Microlearning",
                description: "Aprende los conceptos básicos del Microlearning",
                course_id: 'c4796a20-8f33-42a5-aa9a-2ac3305d9172',
                image:
                    "https://example.com/lesson1.jpg",
                video_url:
                    "https://www.youtube.com/watch?v=eDt4bzeSd7w",
                duration: 15,
                sequence_order: 1,
                created_at: "20-12-2023 15:45hs",
                updated_at: "20-12-2023 15:45hs"
            },
            {
                lesson_id: '6d6e1f94-2a2c-43b7-a596-747f8c5a41f1',
                title: "Ejemplos de Microlearning",
                description: "Descubre ejemplos reales de Microlearning en acción",
                course_id: 'c4796a20-8f33-42a5-aa9a-2ac3305d9172',
                image:
                    "https://example.com/lesson2.jpg",
                video_url:
                    "https://www.youtube.com/watch?v=eDt4bzeSd7w",
                duration: 20,
                sequence_order: 2,
                created_at: "20-12-2023 15:45hs",
                updated_at: "20-12-2023 15:45hs"
            },
            {
                lesson_id: 'edd9dce5-2d6b-4e0a-8c99-12880aa97b61',
                title: "Estrategias de Aprendizaje",
                description: "Explora diferentes estrategias de aprendizaje en el microlearning",
                course_id: 'c4796a20-8f33-42a5-aa9a-2ac3305d9172',
                image:
                    "https://example.com/lesson3.jpg",
                video_url:
                    "https://www.youtube.com/watch?v=eDt4bzeSd7w",
                duration: 25,
                sequence_order: 3,
                created_at: "20-12-2023 15:45hs",
                updated_at: "20-12-2023 15:45hs"
            },
            {
                lesson_id: 'd29cbf0b-2e45-49e4-a488-0b9f4786ad91',
                title: "Evaluación de Microlearning",
                description: "Aprende a evaluar la efectividad del microlearning en tu organización",
                course_id: 'c4796a20-8f33-42a5-aa9a-2ac3305d9172',
                image:
                    "https://example.com/lesson4.jpg",
                video_url:
                    "https://www.youtube.com/watch?v=eDt4bzeSd7w",
                duration: 18,
                sequence_order: 4,
                created_at: "20-12-2023 15:45hs",
                updated_at: "20-12-2023 15:45hs"
            }
        ]
    }, {
        "course_id": "d1a12e5a-62f3-4f7e-9e67-19d8a5abf63e",
        "title": "Curso Avanzado de Programación en Python",
        "description": "Aprende programación en Python desde cero y conviértete en un experto en desarrollo de software.",
        "instructor_id": "e0f83b49-6a3c-4e72-9b15-2cd0b81f7d32",
        "created_at": "25-02-2023 10:30hs",
        "updated_at": "25-02-2023 10:30hs",
        "progress": 0,
        "category": "Programación",
        "image": "https://i.ytimg.com/vi/MHip3eVhK9g/hq720.jpg?sqp=-oaymwEcCOgCEMoBSFXyq4qpAw4IARUAAIhCGAFwAcABBg==&rs=AOn4CLBa2Tc3wl_CWfJzbvhBmHo1sBXdEg",
        "instructorImage": "https://example.com/instructor_python.jpg",
        "rating": 4.5,
        "comments": [
            {
                "user": "a2c5938d-7cfe-4d29-aa7e-3e3a2c5938d",
                "description": "Este curso me ayudó a mejorar mis habilidades en Python. ¡Muy recomendado!"
            },
            {
                "user": "f6e38d9a-2c9d-4a7e-8f2c-7cfe8f9a2c5",
                "description": "El instructor explica de manera clara y concisa. Estoy muy satisfecho con este curso."
            }
        ],
        "lessons": [
            {
                "lesson_id": "f4e3b7d9-62c5-4a1e-9e6a-2c3b6a9d5f4e",
                "title": "Introducción a Python",
                "description": "Conoce los conceptos básicos de Python y cómo configurar tu entorno de desarrollo.",
                "course_id": "d1a12e5a-62f3-4f7e-9e67-19d8a5abf63e",
                "image": "https://example.com/python_intro.jpg",
                "video_url": "https://www.youtube.com/watch?v=eDt4bzeSd7w",
                "duration": 20,
                "sequence_order": 1,
                "created_at": "25-02-2023 10:30hs",
                "updated_at": "25-02-2023 10:30hs"
            },
            {
                "lesson_id": "a6d2c4b3-5e2f-4b7a-9a6f-2b3a6c9d5a6d",
                "title": "Estructuras de Control en Python",
                "description": "Aprende a usar condicionales y bucles en Python para controlar el flujo de tu programa.",
                "course_id": "d1a12e5a-62f3-4f7e-9e67-19d8a5abf63e",
                "image": "https://example.com/python_control.jpg",
                "video_url": "https://www.youtube.com/watch?v=eDt4bzeSd7w",
                "duration": 25,
                "sequence_order": 2,
                "created_at": "25-02-2023 10:30hs",
                "updated_at": "25-02-2023 10:30hs"
            },
            {
                "lesson_id": "c1f2b3e4-7d8e-4a6b-9e9a-3b6a1c9d5f2b",
                "title": "Programación Orientada a Objetos en Python",
                "description": "Descubre los principios de la programación orientada a objetos y cómo aplicarlos en Python.",
                "course_id": "d1a12e5a-62f3-4f7e-9e67-19d8a5abf63e",
                "image": "https://example.com/python_oop.jpg",
                "video_url": "https://www.youtube.com/watch?v=eDt4bzeSd7w",
                "duration": 30,
                "sequence_order": 3,
                "created_at": "25-02-2023 10:30hs",
                "updated_at": "25-02-2023 10:30hs"
            },
            {
                "lesson_id": "b4e9d8c6-5a2f-4e7b-9d8c-3a6b4d9e5a2f",
                "title": "Desarrollo de Aplicaciones Web con Flask",
                "description": "Aprende a crear aplicaciones web con el framework Flask y despliega tus proyectos en línea.",
                "course_id": "d1a12e5a-62f3-4f7e-9e67-19d8a5abf63e",
                "image": "https://example.com/flask_web.jpg",
                "video_url": "https://www.youtube.com/watch?v=eDt4bzeSd7w",
                "duration": 28,
                "sequence_order": 4,
                "created_at": "25-02-2023 10:30hs",
                "updated_at": "25-02-2023 10:30hs"
            }
        ]
    }
    , {
        "course_id": "d1a12e5a-62f3-4f7e-9e67-19d8a5abf63e",
        "title": "Curso Avanzado de Programación en Python",
        "description": "Aprende programación en Python desde cero y conviértete en un experto en desarrollo de software.",
        "instructor_id": "e0f83b49-6a3c-4e72-9b15-2cd0b81f7d32",
        "created_at": "25-02-2023 10:30hs",
        "updated_at": "25-02-2023 10:30hs",
        "progress": 0,
        "category": "Programación",
        "image": "https://i.ytimg.com/vi/MHip3eVhK9g/hq720.jpg?sqp=-oaymwEcCOgCEMoBSFXyq4qpAw4IARUAAIhCGAFwAcABBg==&rs=AOn4CLBa2Tc3wl_CWfJzbvhBmHo1sBXdEg",
        "instructorImage": "https://example.com/instructor_python.jpg",
        "rating": 4.5,
        "comments": [
            {
                "user": "a2c5938d-7cfe-4d29-aa7e-3e3a2c5938d",
                "description": "Este curso me ayudó a mejorar mis habilidades en Python. ¡Muy recomendado!"
            },
            {
                "user": "f6e38d9a-2c9d-4a7e-8f2c-7cfe8f9a2c5",
                "description": "El instructor explica de manera clara y concisa. Estoy muy satisfecho con este curso."
            }
        ],
        "lessons": [
            {
                "lesson_id": "f4e3b7d9-62c5-4a1e-9e6a-2c3b6a9d5f4e",
                "title": "Introducción a Python",
                "description": "Conoce los conceptos básicos de Python y cómo configurar tu entorno de desarrollo.",
                "course_id": "d1a12e5a-62f3-4f7e-9e67-19d8a5abf63e",
                "image": "https://example.com/python_intro.jpg",
                "video_url": "https://www.youtube.com/watch?v=eDt4bzeSd7w",
                "duration": 20,
                "sequence_order": 1,
                "created_at": "25-02-2023 10:30hs",
                "updated_at": "25-02-2023 10:30hs"
            },
            {
                "lesson_id": "a6d2c4b3-5e2f-4b7a-9a6f-2b3a6c9d5a6d",
                "title": "Estructuras de Control en Python",
                "description": "Aprende a usar condicionales y bucles en Python para controlar el flujo de tu programa.",
                "course_id": "d1a12e5a-62f3-4f7e-9e67-19d8a5abf63e",
                "image": "https://example.com/python_control.jpg",
                "video_url": "https://www.youtube.com/watch?v=eDt4bzeSd7w",
                "duration": 25,
                "sequence_order": 2,
                "created_at": "25-02-2023 10:30hs",
                "updated_at": "25-02-2023 10:30hs"
            },
            {
                "lesson_id": "c1f2b3e4-7d8e-4a6b-9e9a-3b6a1c9d5f2b",
                "title": "Programación Orientada a Objetos en Python",
                "description": "Descubre los principios de la programación orientada a objetos y cómo aplicarlos en Python.",
                "course_id": "d1a12e5a-62f3-4f7e-9e67-19d8a5abf63e",
                "image": "https://example.com/python_oop.jpg",
                "video_url": "https://www.youtube.com/watch?v=eDt4bzeSd7w",
                "duration": 30,
                "sequence_order": 3,
                "created_at": "25-02-2023 10:30hs",
                "updated_at": "25-02-2023 10:30hs"
            },
            {
                "lesson_id": "b4e9d8c6-5a2f-4e7b-9d8c-3a6b4d9e5a2f",
                "title": "Desarrollo de Aplicaciones Web con Flask",
                "description": "Aprende a crear aplicaciones web con el framework Flask y despliega tus proyectos en línea.",
                "course_id": "d1a12e5a-62f3-4f7e-9e67-19d8a5abf63e",
                "image": "https://example.com/flask_web.jpg",
                "video_url": "https://www.youtube.com/watch?v=eDt4bzeSd7w",
                "duration": 28,
                "sequence_order": 4,
                "created_at": "25-02-2023 10:30hs",
                "updated_at": "25-02-2023 10:30hs"
            }
        ]
    }, {
        course_id: 'd2390d33-4c57-4a62-9a8b-47c9a0e3d7e3',
        title: "Curso de Marketing Digital",
        description: "Aprende estrategias efectivas de marketing en línea",
        instructor_id: '8a5b3c9e-2b3d-4e8a-8d9c-0b7f5d6a8d9c',
        created_at: "15-01-2024 14:30hs",
        updated_at: "15-01-2024 14:30hs",
        progress: 0,
        category: "Marketing",
        image:
            "https://example.com/marketing-course.jpg",
        instructorImage:
            "https://example.com/instructor-marketing.jpg",
        rating: 4.5,
        comments: [
            {
                user: '0e3c7e8d-4a62-4c8b-9a7c-8a0e3d9c5d2b',
                description: "Gran curso para aprender marketing digital."
            },
            {
                user: '4e2d4a1e-4a6b-4b8d-9e8c-4b8f6a6a8e9d',
                description: "Muy útil para aquellos que deseen promocionar sus negocios en línea."
            }
        ],
        lessons: [
            {
                lesson_id: 'e4c2c6a6-2b2a-4b6d-b8ab-d0e8b6eb8e5d',
                title: "Introducción al Marketing Digital",
                description: "Conceptos básicos y estrategias iniciales de marketing digital",
                course_id: 'd2390d33-4c57-4a62-9a8b-47c9a0e3d7e3',
                image:
                    "https://example.com/marketing-lesson1.jpg",
                video_url:
                    "https://example.com/marketing-lesson1.mp4",
                duration: 20,
                sequence_order: 1,
                created_at: "15-01-2024 14:30hs",
                updated_at: "15-01-2024 14:30hs",
            },
            {
                lesson_id: '4b6e2f9d-2a3c-4a6e-8b5d-5f4b6a4f1a6f',
                title: "Publicidad en Redes Sociales",
                description: "Cómo aprovechar las redes sociales para promocionar tu negocio",
                course_id: 'd2390d33-4c57-4a62-9a8b-47c9a0e3d7e3',
                image:
                    "https://example.com/marketing-lesson2.jpg",
                video_url:
                    "https://example.com/marketing-lesson2.mp4",
                duration: 25,
                sequence_order: 2,
                created_at: "15-01-2024 14:30hs",
                updated_at: "15-01-2024 14:30hs",
            },
            {
                lesson_id: '8e9d7c9e-3d4e-4a6c-8b5c-9e7a8e0e3d9c',
                title: "SEO y Optimización Web",
                description: "Mejora la visibilidad de tu sitio web en los motores de búsqueda",
                course_id: 'd2390d33-4c57-4a62-9a8b-47c9a0e3d7e3',
                image:
                    "https://example.com/marketing-lesson3.jpg",
                video_url:
                    "https://example.com/marketing-lesson3.mp4",
                duration: 30,
                sequence_order: 3,
                created_at: "15-01-2024 14:30hs",
                updated_at: "15-01-2024 14:30hs",
            }
        ]
    }
];

